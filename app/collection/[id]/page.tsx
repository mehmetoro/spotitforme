...
export default function CollectionDetailPage() {
  ...
  useEffect(() => {
    const loadPage = async () => {
      try {
        ...
        if (!id) {
          setError('Koleksiyon kaydı bulunamadı')
          return
        }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (editPhotoFile && item?.user_id) {
        const fileExt = editPhotoFile.name.split('.').pop() || 'jpg'
        const fileName = `${item.user_id}/collection/${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`
      ...
      if (updateError || !data) {
        throw updateError || new Error('Kayıt güncellenemedi')
      }
  ...
  if (!item || error) {
    return (
      <main className="container-custom py-10 text-center">
        ...
        <Link href="/collection" className="text-blue-600 hover:text-blue-800 font-medium">
  ...
  return (
    <main className="container-custom py-8">
      <div className="mb-6">
        <Link href="/collection" className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-medium text-sm">
          ← Koleksiyon listesine dön
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
          <div className="bg-gray-100 min-h-[280px]">
            {item.photo_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-6xl text-blue-300">🧳</div>
            )}
          </div>
          ...
          <div className="p-6 space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              {item.category && (
                <span className="bg-gray-100 text-gray-700 text-xs px-3 py-1 rounded-full">{item.category}</span>
              )}
              {!item.is_public && (
                <span className="bg-amber-100 text-amber-700 text-xs px-3 py-1 rounded-full">Özel Kayıt</span>
              )}
            </div>
            ...
            {editMode ? (
              <div className="space-y-3 border border-blue-100 bg-blue-50 rounded-xl p-4">
                  ...
                  {(editPhotoPreview || editForm.photo_url) && (
                    <div className="mt-3">
                      <img
                        ...
                        alt="Koleksiyon fotoğraf önizleme"
                ...
                <button
                  ...
                  disabled={saving}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 rounded-lg disabled:opacity-60"
                >
            ...
            {item.estimated_price != null && (
              <p className="text-lg font-semibold text-green-700">
                Tahmini Değer: ₺{item.estimated_price.toLocaleString('tr-TR')}
              </p>
            )}
            ...
            <div className="border-t pt-4">
              <div className="flex items-center gap-3 mb-3">
                {owner?.avatar_url ? (
                  <img src={owner.avatar_url} alt={owner.full_name || 'Kullanıcı'} className="w-11 h-11 rounded-full object-cover" />
                ) : (
              ...
              {item.user_id !== currentUserId && (
                <>
                  <p className="text-xs text-gray-500 mb-2">
                    Bu paylaşım için mesaj talebi gönderildiğinde profesyonel/ticari iş birliklerinde varsayılan
                    <span className="font-semibold text-gray-700"> %15 yardım komisyonu</span> modeli uygulanabilir.
                  </p>
                  <button
